...
const getStatusBadge = (status: string) => {
	...
	return (
		<span className={`px-2.5 py-0.5 rounded-full text-xs font-medium border ${colorClass} capitalize`}>
			{status}
		</span>
	);
};
...
export default function SalesListPage() {
	...
	const stats = useMemo(() => {
		...
		const overdue = filteredInvoices.filter((inv) => inv.status.toLowerCase() === 'overdue').length;
	...
	if (loading) {
		return (
			<div className='p-6 bg-[#0C0C0E] min-h-screen text-gray-400 flex items-center justify-center'>
	...
	return (
		<div className='p-6 bg-[#0C0C0E] min-h-screen text-gray-100 font-sans'>
			{/* HEADER */}
			<div className='flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8'>
				<div>
					<h1 className='text-3xl font-bold tracking-tight'>Sales Invoices</h1>
					<p className='text-gray-400 text-sm'>Track your revenue and manage customer billing</p>
				</div>
				<Link
					href='/sales/invoices'
			...
			<div className='grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8'>
				<div className='bg-gray-800/40 border border-gray-700/50 p-5 rounded-xl shadow-sm'>
					<p className='text-gray-400 text-xs font-bold uppercase tracking-wider mb-1'>Total Sales</p>
					<h3 className='text-2xl font-black text-white'>{formatCurrency(stats.total)}</h3>
					<p className='text-gray-500 text-xs mt-2'>{stats.count} Invoices generated</p>
				</div>
				<div className='bg-gray-800/40 border border-gray-700/50 p-5 rounded-xl shadow-sm'>
					<p className='text-gray-400 text-xs font-bold uppercase tracking-wider mb-1'>Total Collected</p>
					<h3 className='text-2xl font-black text-green-400'>{formatCurrency(stats.paid)}</h3>
					<p className='text-gray-500 text-xs mt-2'>Revenue in bank</p>
				</div>
				<div className='bg-gray-800/40 border border-gray-700/50 p-5 rounded-xl shadow-sm'>
					<p className='text-gray-400 text-xs font-bold uppercase tracking-wider mb-1'>Receivables</p>
					<h3 className='text-2xl font-black text-blue-400'>{formatCurrency(stats.total - stats.paid)}</h3>
					<p className='text-gray-500 text-xs mt-2'>Outstanding balance</p>
				</div>
				<div className='bg-gray-800/40 border border-gray-700/50 p-5 rounded-xl shadow-sm'>
					<p className='text-gray-400 text-xs font-bold uppercase tracking-wider mb-1'>Overdue</p>
			...
			{/* FILTERS */}
			<div className='bg-gray-800/30 border border-gray-700/50 p-4 rounded-xl mb-6 flex flex-col md:flex-row gap-4'>
				<div className='flex-1 relative'>
					...
					<input
						...
						className='w-full pl-10 pr-4 py-2 bg-gray-900/50 border border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all'
					/>
				</div>
				<div className='flex gap-2'>
					<select
						...
						className='px-4 py-2 bg-gray-900/50 border border-gray-700 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none transition-all'
					>
			...
			{/* TABLE */}
			<div className='bg-gray-800/40 border border-gray-700/50 rounded-xl overflow-hidden shadow-xl'>
				{error ? (
						...
						<table className='w-full text-left border-collapse'>
							...
							<tbody className='divide-y divide-gray-700/50'>
								{filteredInvoices.map((inv) => (
									<tr key={inv.id} className='hover:bg-gray-700/20 transition-colors group'>
										...
										<td className='px-6 py-4'>
											<p className='font-semibold text-gray-200'>{inv.billToCompany}</p>
										</td>
